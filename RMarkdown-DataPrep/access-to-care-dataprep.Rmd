...
```{r}
con <- connection_open(RSQLite::SQLite(), "/tmp/local.sqlite")
copy_to(con, county_shapes, "shapes", overwrite = TRUE, temporary = FALSE)
copy_to(con, county_hospitals, "counties", overwrite = TRUE, temporary = FALSE)
copy_to(con, hospital_locations, "hospitals", overwrite = TRUE, temporary = FALSE)
copy_to(con, state_full, "states", overwrite = TRUE, temporary = FALSE)
```

```{r} 
base_map <-  tbl(con, "states") %>%
  select(state_id, full, state) %>%
  inner_join(tbl(con, "counties"), by = "state_id") %>%
  mutate(
...
```{r} 
county_map <- base_map %>% 
  left_join(tbl(con, "shapes"), by = "county_id") %>%
  filter(!is.na(county)) 
```
