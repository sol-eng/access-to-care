...
st_locations <- st_locations_list %>% 
  map_dfr(~.x)
...
gp <- us_hex_map %>% 
  ggplot() +
  geom_polygon_interactive(aes(x, y, group = group, data_id = group), fill = "lightgray", color = "white") +
  geom_text(aes(x, y, label = state), data = st_locations) +
  theme_void() +
...
```{r}
plot_usmap(regions = "county", data = us_counties, values = "pred_status") +
  scale_fill_manual(breaks = c("below", "above", "ok"), values = c("red", "blue", "green")) +
