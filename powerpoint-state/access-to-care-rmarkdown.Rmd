...
cts <- counties %>%
  ...
  map_df(
    ~.x$data %>% 
      ...
      map_df(
        ~.x$data %>% 
          mutate(shape_id = .x$shape_id)) %>%
      mutate(
        county = .x$county,
        color = .x$color,
        group = paste(county, shape_id)
        ))
...
cts_map <- ggplot(cts) +
  ...
  labs(title = "Breakdown of counties and their status based on county population compared to the number of hospitals
")
